...
const excelFile = xlsx.readFile('src/common/ns홈쇼핑.xlsx');
...
const jsonParser = (array) => {
    ...
    jsonData.map(v => {
        ...
        newList.push(v.고객명); //주문자
        newList.push(v.고객전화번호); //주문자전화
        newList.push(v.고객휴대폰번호); //주문자핸드폰
        newList.push('') //주문자이메일
        newList.push(v.수취인명실주문자명); //수령자
        newList.push(v.수취인전화번호); //전화
        newList.push(v.수취인휴대폰번호); //핸드폰
