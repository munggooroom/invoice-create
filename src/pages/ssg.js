...
const excelFile = xlsx.readFile('src/common/ssg.xlsx');
...
const jsonParser = (array) => {
    ...
    jsonData.map(v => {
        ...
        newList.push('[SSG])'+v.상품명); //상품명
        newList.push(v.옵션명); //옵션
        ...
        newList.push(v.수취인휴대전화번호); //핸드폰
        newList.push('') //수령자영문이름
        newList.push('') //수령자주민등록번호(통관용)
        newList.push(v.우편번호); //우편번호
        newList.push(v.수취인도로명주소 || v.수취인지번주소); //주소
        newList.push(v.고객배송메모); //배송메세지
        newList.push('') //배송사명
        newList.push('') //송장번호
        newList.push('') //사은품
...
//시트 생성
xlsx.utils.book_append_sheet(list, orders, 'SSG송장');

//엑셀 파일 생성
xlsx.writeFile(list, 'SSG송장.xlsx');